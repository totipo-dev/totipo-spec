...
help:
	@printf '%s\n' 'make spec-check  Check the v1/r11 specification structure' 'make test        Run Go tests' 'make conformance Run all v1 cases' 'make verify      Verify manifest, files, and moving profile' 'make check       Run all required checks' 'make race        Run Go race tests' 'make fuzz        Run bounded parser fuzzing'
